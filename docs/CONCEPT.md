...
Работаем с командой в нескольких проектах и на разных платформах параллельно. В каждом проекте есть документация, и между проектами много пересечений:
...
- **Divergence (v0.9.3):** отдельная ось состояния карточки, ортогональная `stale`. Когда один из экземпляров группы дублей отредактирован — его `content_hash` уходит из группы, остальные копии остаются. Такой карточке присваивается `diverged_from` — список бывших siblings, с кем hash больше не совпадает. Флаг носит только отредактированная карточка (неизменённые siblings чисты). Снимается автоматически при reindex, когда хеш снова совпадает хотя бы с одной из бывших копий (частичная конвергенция сужает список), либо вручную через `accept_divergence(doc_id)` для осознанного форка
...
- **Web UI** — `.venv/bin/python -m astrolabe.web`, просмотр и редактирование карточек через браузер. Работает параллельно с MCP-сервером, общий storage
...
- **Transport: HTTP** — Web UI (v0.8.0). FastAPI + Jinja2 + HTMX. Для просмотра и редактирования карточек через браузер
...
- **Зависимости:** минимум (только `mcp` SDK). Web UI — опциональные: FastAPI, Jinja2, HTMX, mistune (`[web]`)

~~**Вопрос:** Сможем ли мы потом транспорт расширить и на HTTP/CLI?~~ **Решено:** HTTP реализован как Web UI (v0.8.0)
...
- ~~HTTP / CLI транспорты~~ → HTTP реализован (v0.8.0 Web UI). CLI — будущее
- ~~Детекция дубликатов (один реф скопирован в 3 проекта)~~ → реализована (v0.7.2, on-the-fly через content_hash)
- ~~Детекция расщепления группы дублей при правке одного экземпляра~~ → реализована (v0.9.3, флаг `diverged_from`, ручной `accept_divergence` или авто-reconvergence)
